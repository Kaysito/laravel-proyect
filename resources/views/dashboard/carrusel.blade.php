...
<div class="max-w-4xl mx-auto">
    ...
    <div class="relative w-full overflow-hidden rounded-2xl shadow-2xl border border-slate-200 bg-slate-900 mb-10 group">
        ...
        {{-- CONTROLES --}}
        @if(!empty($imagenes) && count($imagenes) > 1)
            <button aria-label="Imagen anterior"
                onclick="prevSlide()"
                class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-3 rounded-full transition transform hover:scale-110 opacity-0 group-hover:opacity-100">
                <i class="fa-solid fa-chevron-left"></i>
            </button>

            <button aria-label="Imagen siguiente"
                onclick="nextSlide()"
                class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-3 rounded-full transition transform hover:scale-110 opacity-0 group-hover:opacity-100">
            ...
            <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                @foreach($imagenes as $index => $img)
                    <button
                        ...
                        {{ $index === 0 ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/80' }}">
                    </button>
                @endforeach
            </div>
        @endif
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-8">
        ...
        <form id="uploadForm"
            ...
            <div id="file-name-display"
                 class="hidden mb-4 text-sm text-center text-emerald-600 font-medium bg-emerald-50 py-2 px-4 rounded">
                <i class="fa-solid fa-image mr-2"></i>
                <span id="file-name-text"></span>
            </div>

            <button id="submitBtn"
                    class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl shadow-lg transition active:scale-95">
